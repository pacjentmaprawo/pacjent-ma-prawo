...
export default function MethodPageRoute() {
  return (
    <MethodPage
      method={method}
      what={<><p>Polifenol z korzenia kurkumy długiej (Curcuma longa). W formie standardowej ma <strong>bardzo niską biodostępność doustną</strong> (&lt; 1%) — dlatego stosuje się formy <strong>liposomalne, fosfolipidowe (Meriva), nano-dyspersyjne</strong> lub w kombinacji z piperyną (BioPerine), które zwiększają wchłanianie 5-20-krotnie.</p><div className="my-4 rounded-md border border-amber-300 bg-amber-50 p-4 text-sm"><p className="font-semibold mb-2">Paradoks medialny — argument „niska biodostępność = brak skuteczności"</p><p>Częsta krytyka kurkuminy w mediach opiera się na danych biodostępności <em>standardowej</em> kurkuminy proszkowej (&lt;1%). To prawda dla zwykłego ekstraktu — ale ignoruje, że <strong>aktualne badania kliniczne stosują formy wystandaryzowane</strong> (liposomalne, fosfolipidowe Meriva, micelarne, BioPerine) o biodostępności 5–20× wyższej. Howells 2019 (PMID 31132111) w mCRC + FOLFOX używał właśnie standaryzowanej formy. Krytyka „kurkumina nie działa, bo ma niską biodostępność" pomija dystynkcję między formą surową a standaryzowaną — to klasyczny błąd argumentacyjny „pomijanie standaryzacji preparatu".</p></div></>}
